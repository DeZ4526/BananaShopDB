...
BEGIN
	IF ((SELECT Count(*) FROM users WHERE id_user = _id_user) = 1 AND
	   ...
	   (SELECT balance FROM Users WHERE  id_user = _id_user) >= (SELECT price FROM Products WHERE _id_product = id_product)) THEN
		INSERT INTO Orders (id_user, id_product ,information, date_time, is_done) VALUES 
		(_id_user, _id_product,_information, now(), false);
		
		SELECT Add_Balance_ref((SELECT id_referral FROM Users WHERE id_user = _id_user), 
							   _id_user, 
							   ((SELECT price FROM Products WHERE _id_product = id_product)/ 100 * ref_percent));
...
CREATE FUNCTION Add_Balance_ref(_id_user int, _requisites text, _value numeric) RETURNS bool AS $$
BEGIN
	...
	IF(SELECT count(*) FROM Purchase_methods WHERE title='REFERAL_SYSTEM') = 0 THEN
		SELECT Add_Purchase_method('REFERAL_SYSTEM', 'REFERAL_SYSTEM');
	END IF;
	
	INSERT INTO Balance_log (id_user, id_p_method ,requisites ,value ,description ,date_time,use_bonus) VALUES 
	(_id_user, (SELECT SUM(id_p_method) FROM Purchase_methods WHERE title='REFERAL_SYSTEM'),_requisites,_value,'REFERAL_SYSTEM', now(), false);
